...
Now you can use the API to parse NTriples files. Check the example at example.rb.


== Deploying

The deploy process is based on capistrano and foreman. Before deploying, create a file called servers.rb at the root of the project. In this file, you should list your servers as you would with capistrano, using the :foreman role for your resque-web instance and workers. For example:
...
On the first time you deploy, run:

$ cap deploy:cold

To avoid errors when starting foreman to take care of the processes. Finally, everytime later you wish to deploy just run:
